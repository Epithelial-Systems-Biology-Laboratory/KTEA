...
dens <- density(prot_trans_correlation$correlation_pearson,na.rm = TRUE)
...
ggplot(df, aes(x,y)) + 
  ...
  theme_bw() +
  xlab("Gene-wise protein-mRNA Pearson correlation") +
  ylab("Density") +
  geom_vline(xintercept = median(prot_trans_correlation$correlation_pearson,na.rm = TRUE), linetype=2, color="white") + 
  geom_text(x=-0.6,y=1.1,color="black", size=10/.pt, 
            aes(label=paste0("Median: ",
                             round(median(prot_trans_correlation$correlation_pearson,na.rm = TRUE),2),
