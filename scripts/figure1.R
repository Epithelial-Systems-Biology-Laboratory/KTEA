...
transporter_list <- read_tsv(snakemake@input[['transporter_list']])
proteome <- read_csv(snakemake@input[['proteome']]) %>% mutate(segment=fct_relevel(segment,tubule_segments))

transporters <- proteome %>% 
  filter(gene %in% transporter_list$`Gene Symbol`) %>% 
  ...
  left_join(transporter_list, by=c("gene"="Gene Symbol")) %>% 
  ungroup()

transporters %>% 
  gather(segment,value, -gene, -`Substrate(s)`) %>% 
  mutate(segment=case_when(segment=="CTAL" ~ "cTAL",
                           segment=="MTAL" ~ "mTAL",
                           TRUE ~ segment)) %>% 
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`,"-", '\\^\\{\\"-\\"\\}')) %>% 
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`,"\\+", "\\^\\{\\'\\+\\'\\}")) %>% 
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`,"Mg.*", "Mg\\^\\{\\'2\\+\\'\\}")) %>% 
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`,"HCO3", "HCO\\[3\\]")) %>%
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`,"NH3", "NH\\[3\\]")) %>% 
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`,"NH4", "NH\\[4\\]")) %>% 
  mutate(`Substrate(s)`= str_replace_all(`Substrate(s)`," ", "~")) %>% 
  ggplot(aes(x=fct_relevel(segment,tubule_segments),fct_rev(gene),fill=value)) + 
  geom_tile(show.legend = FALSE) + 
  geom_text(aes(label=round(value,2)), size = 7/.pt) +
  geom_label(aes(label=`Substrate(s)`, x=16.5),size=7/.pt,label.padding = unit(.2,"lines"), fill=NA, parse = TRUE) +
  theme_minimal() +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#1D6996")), 
                    xmin=0.5,xmax=3.5,ymin=33.7, ymax=34.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#0F8554")), 
                    xmin=3.5,xmax=7.5,ymin=33.7, ymax=34.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#EDAD08")), 
                    xmin=7.5,xmax=9.5,ymin=33.7, ymax=34.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#CC503E")), 
                    xmin=9.5,xmax=10.5,ymin=33.7, ymax=34.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#6F4070")), 
                    xmin=10.5,xmax=14.5,ymin=33.7, ymax=34.7) +
