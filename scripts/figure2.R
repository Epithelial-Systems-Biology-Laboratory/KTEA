...
metabolic_enzyme <- read_tsv(snakemake@input[['metabolic_enzyme_list']])
proteome <- read_csv(snakemake@input[['proteome']]) %>% mutate(segment=fct_relevel(segment,tubule_segments))

metabolic_enzyme <- proteome %>% 
  filter(gene %in% metabolic_enzyme$`Gene Symbol`) %>% 
  ...
  left_join(metabolic_enzyme, by=c("gene"="Gene Symbol")) %>% 
  ungroup()



metabolic_enzyme %>% 
  gather(segment,value, -gene, -`Functional Role`) %>% 
  ...
  geom_label(aes(label=`Functional Role`, x=16.5),size=7/.pt,label.padding = unit(.2,"lines"), fill=NA) +
  theme_minimal() +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#1D6996")), 
                    xmin=0.5,xmax=3.5,ymin=27.7, ymax=28.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#0F8554")), 
                    xmin=3.5,xmax=7.5,ymin=27.7, ymax=28.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#EDAD08")), 
                    xmin=7.5,xmax=9.5,ymin=27.7, ymax=28.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#CC503E")), 
                    xmin=9.5,xmax=10.5,ymin=27.7, ymax=28.7) +
  ...
  annotation_custom(grid::rectGrob(gp = grid::gpar(fill="#6F4070")), 
                    xmin=10.5,xmax=14.5,ymin=27.7, ymax=28.7) +
